...
exports.signup = async (req, res, next) => {
  ...
  try {
    ...
    const users = await pool.query(
      `SELECT * FROM ${tables.user} WHERE email=?`,
      [email]
    )
    ...
    const result = await pool.query(
      `INSERT INTO ${tables.user} SET email=?,password=?`,
      [email, password, name]
    )
    if (result.affectedRows) {
      await pool.query(
        `INSERT INTO ${tables.profile} SET user_id=?,name=?,phone=?`,
...
exports.login = async (req, res, next) => {
  ...
  try {
    ...
    result[0].image_url =
      req.protocol + "://" + req.get("host") + "/" + result[0].image_url
...
exports.profile = async (req, res, next) => {
  ...
  try {
    ...
    userProfile[0].joined_date = moment(userProfile[0].joined_date).format(
      "MMMM Do YY"
    )
...
exports.changepw = async (req, res, next) => {
  ...
  try {
    ...
    if (user.length > 0) {
      ...
      const newHashedPw = await bcrypt.hash(
        newPassword,
        parseInt(process.env.SALT_ROUNDS, 10)
      )
...
exports.uploadPicture = async (req, res, next) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE ${tables.profile} SET image_url=? WHERE user_id=?`,
...
exports.getForgotCode = async (req, res, next) => {
  ...
  const results = await pool.query(
    `SELECT email FROM ${tables.user} WHERE email=?`,
    [email]
  )
  if (results.length > 0) {
    const reset_token = Math.floor(100000 + Math.random() * 900000)
    await pool.query(`UPDATE ${tables.user} SET reset_token=? WHERE email=?`, [
      reset_token,
      email
    ])
    return res.json({
